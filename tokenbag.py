...
class TokenBag:
  ...
  def _replayable_pull(self) -> list:
    ...
    for bag in bag_pulls:
      ...
      flippedEnd = (
          (token["Can Flip"] and token["Flipped"]["Ends Draws"])
          or (not token["Can Flip"] and token["Ends Draws"])
      )
      if flippedEnd:
        break
    ...
    while draw_again:
      ...
      if token["Can Steal"] and hasRank:
        ...
        for r in canBeStolen:
          ...
          rHasFlippedRank = True if (rToken["Can Flip"] and rToken["Flipped"]["Min Rank"] <= rank) else False
        ...
        for r in canBeStolenFlipped:
          rToken = self.pool["tokens"][r]
          rHasFlippedRank = False if not rToken["Can Flip"] else (
            rToken["Flipped"]["Min Rank"] <= rank)
      ...
      if hasFlippedRank and token["Flipped"]["Can Latch"] or (
          token["Can Latch"] and not hasFlippedRank):
        latchedFlipped = True
      ...
      # Check if token ends draws
      if token["Ends Draws"]:
        # The replayable pull ends when no more tokens can be drawn for fortune pulls
        # If only the base pull is ending here, we still need to process the fortune pull
        baseDrawEnded = True

      if rs["misses"] >= self.miss_ceil or rs["hits"] >= self.hit_ceil:
        if (self.hit_ceil_only_on_crit and canCrit) or not self.hit_ceil_only_on_crit:
          finalHitMiss = True
      ...
      if rs["fortune-misses"] >= self.miss_ceil or rs["fortune-hits"] >= self.hit_ceil:
        if self.hit_ceil_only_on_crit and canCrit or not self.hit_ceil_only_on_crit:
          finalFlippedHitMiss = True
      if rs["fortune-sum"] >= self.sum_ceil or rs["fortune-sum"] <= self.sum_floor:
